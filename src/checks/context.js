 ...
 *
 * Campaign-UI stores the JWT in sessionStorage["token"] which Playwright's
 * storageState() does not capture. Rather than trying to inject sessionStorage
 * (which is unreliable), we perform a full login inside each new context when
 * routes require auth. The session is cached to disk after the first login so
 * subsequent contexts reuse it without hitting the login page again.
 */
